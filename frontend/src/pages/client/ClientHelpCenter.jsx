...
Access Settings > Notifications to configure your preferences.`
  ...
  const renderArticleContent = (article) => {
    ...
    return lines.map((line, index) => {
      if (line.startsWith('## ')) return <h2 key={index} className="text-2xl font-bold text-gray-800 mt-6 mb-4">{line.replace('## ', '')}</h2>;
      if (line.startsWith('### ')) return <h3 key={index} className="text-lg font-semibold text-gray-700 mt-4 mb-2">{line.replace('### ', '')}</h3>;
      if (line.startsWith('- ')) return <li key={index} className="ml-4 text-gray-600">{line.replace('- ', '')}</li>;
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
          <div className="flex gap-1 p-1">
            {[{ id: 'guides', label: 'Guides & Documentation', icon: <BookOpen className="w-4 h-4" /> }, { id: 'videos', label: 'Video Tutorials', icon: <Video className="w-4 h-4" /> }].map(tab => (
              <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-medium text-sm transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>
                {tab.icon}
                {tab.label}
              </button>
            ))}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sticky top-4">
                ...
                <nav className="space-y-1">
                  {filteredSections.map(section => (
                    <div key={section.id}>
                      <button onClick={() => toggleSection(section.id)} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-colors ${expandedSections.includes(section.id) ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-50'}`}>
                      ...
                      {expandedSections.includes(section.id) && (
                        <div className="ml-10 mt-1 space-y-0.5 border-l-2 border-gray-200 pl-3">
                          {section.articles.map(article => (
                            <button key={article.id} onClick={() => setSelectedArticle(article)} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${selectedArticle?.id === article.id ? 'bg-blue-100 text-blue-700 font-medium' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}>
                              {article.title}
                            </button>
                          ))}
                        </div>
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  <div className="space-y-2">
                    <Link to="/client/faq" className="flex items-center gap-2 text-sm text-gray-600 hover:text-blue-600 transition-colors"><HelpCircle className="w-4 h-4" />FAQ</Link>
                    <Link to="/client/chat" className="flex items-center gap-2 text-sm text-gray-600 hover:text-blue-600 transition-colors"><MessageCircle className="w-4 h-4" />Contact Support</Link>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0">
              {selectedArticle ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                  <button onClick={() => setSelectedArticle(null)} className="flex items-center gap-1 text-blue-600 hover:text-blue-700 text-sm font-medium mb-6"><ArrowLeft className="w-4 h-4" />Back to all topics</button>
                  <div className="flex items-center gap-3 mb-6">
                    <h1 className="text-2xl font-bold text-gray-800">{selectedArticle.title}</h1>
                    <span className="flex items-center gap-1 text-gray-500 text-sm bg-gray-100 px-3 py-1 rounded-full"><Clock className="w-4 h-4" />{selectedArticle.readTime} read</span>
                  </div>
                  ...
                  {filteredSections.map(section => (
                    <div key={section.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                      ...
                      <div className="grid sm:grid-cols-2 gap-3">
                        {section.articles.map(article => (
                          <button key={article.id} onClick={() => { setSelectedArticle(article); if (!expandedSections.includes(section.id)) toggleSection(section.id); }} className="flex items-center justify-between p-4 rounded-xl border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-all text-left group">
                            <div>
                              <span className="text-gray-800 font-medium text-sm group-hover:text-blue-700">{article.title}</span>
                              <span className="flex items-center gap-1 text-gray-500 text-xs mt-1"><Clock className="w-3 h-3" />{article.readTime}</span>
                            </div>
          ...
          <div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filterContent(videoTutorials, searchQuery).map(video => (
                <div key={video.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all group cursor-pointer">
                  ...
                  <div className="p-4">
                    <span className="text-xs font-medium text-blue-600 bg-blue-50 px-2 py-1 rounded">{video.category}</span>
                    <h3 className="text-gray-800 font-semibold mt-2 group-hover:text-blue-600 transition-colors">{video.title}</h3>
