...
import {
  ...
  ArrowLeft, Check, CheckCheck, Paperclip, Smile
} from 'lucide-react';
...
const ClientChatPage = () => {
  ...
  useEffect(() => {
    // If no conversations, try to find an admin user
  ...
  const findAdminUser = async () => {
    try {
      ...
      if (admins.length > 0) {
        const admin = admins[0]; // Get first admin
        setAdminUser(admin);
        setSelectedChat({
          id: 'new',
          other_user: {
  ...
  const fetchConversations = async () => {
    try {
      ...
      // For clients, auto-select the admin chat
  ...
  const fetchMessages = async (userId, showLoading = true) => {
    try {
      ...
      if (showLoading) setLoading(false);
      // If error, still set loading to false and show empty messages
      setMessages([]);
      if (showLoading) setLoading(false);
    }
  };
  ...
  const sendMessage = async (e) => {
    ...
    const receiverId = selectedChat?.other_user?.id || adminUser?.id;
  ...
  return (
    <ClientDashboardLayout>
      <div className="h-[calc(100vh-80px)] flex bg-white">
        {/* Conversations List - For clients, show admin chat */}
        {user.role === 'CLIENT' ? (
          // Client view: Just show the admin chat directly
            ...
            <div className="h-16 border-b border-gray-200 flex items-center px-6 bg-white">
              <div className="flex items-center gap-3 flex-1">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white font-semibold">
                  {selectedChat ? getUserInitials(selectedChat.other_user.full_name) :
                   adminUser ? getUserInitials(adminUser.full_name || adminUser.username) : 'A'}
                </div>
                <div>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-gray-500 max-w-sm">
                    Send a message to the administrator and get support.
                  </p>
                </div>
              ) : (
            ...
            <div className="h-16 border-t border-gray-200 px-4 flex items-center gap-2 bg-[#f0f2f5]">
              ...
              <form onSubmit={sendMessage} className="flex-1 flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 rounded-full bg-white border-none focus:outline-none focus:ring-2 focus:ring-green-500"
                />
                <button
                  type="submit"
          ...
          <>
            ...
            {selectedChat ? (
              <div className="flex-1 flex flex-col">
                ...
                {/* Messages Area - Same as client view */}
                <div
                  className="flex-1 overflow-y-auto p-6 bg-[#f0f2f5]"
                  style={{
                    backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23d4d4d8' fill-opacity='0.1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`
