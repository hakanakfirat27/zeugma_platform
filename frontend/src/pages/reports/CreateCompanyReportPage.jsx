// frontend/src/pages/reports/CreateCompanyReportPage.jsx
/**
 * Create Company Report Page
 * 
 * Similar to CreateReportPage.jsx but for Company Database with:
 * - Company Status filter (Complete, Incomplete, Deleted)
 * - Companies can have multiple processes/categories
 * - Uses CompanyFilterSidebar instead of FilterSidebarWithGroups
 */

import React, { useState, useEffect } from 'react';
import { useNavigate, useLocation, useParams } from 'react-router-dom';
import { getBreadcrumbs } from '../../utils/breadcrumbConfig';
import { ArrowLeft, X, Search, ChevronRight, CheckCircle2, Filter, BarChart3, Globe, Layers, SlidersHorizontal, Building2 } from 'lucide-react';
...
const CreateCompanyReportPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    is_active: true,
    filter_criteria: {},
    source_type: 'COMPANY_DATABASE' // Indicates this is from Company Database
  });
  ...
  const [statusFilters, setStatusFilters] = useState(['COMPLETE', 'INCOMPLETE']); // NEW: Company status
  ...
  // Fetch filter options and technical filter options
  ...
  // Update availableFilters when filterOptions changes
  useEffect(() => {
    setAvailableFilters(filterOptions);
  }, [filterOptions]);

  // Available categories for CompanyFilterSidebar
  ...
  const fetchReportData = async () => {
    try {
      setLoading(true);
      // Use existing custom-reports API with database_source identifier
      ...
      setFormData({
        ...
        source_type: report.source_type || 'COMPANY_DATABASE'
      });
  ...
  }, [selectedCategories, filterGroups, countryFilters, categoryFilters, statusFilters]);

  // Handle preloaded filters from CompanyDatabasePage
  useEffect(() => {
    if (Object.keys(preloadedFilters).length > 0) {
      console.log('🔄 Preloaded Filters:', preloadedFilters);

      // Handle status filters
      if (preloadedFilters.status) {
        if (Array.isArray(preloadedFilters.status)) {
          setStatusFilters(preloadedFilters.status);
        } else {
          setStatusFilters([preloadedFilters.status]);
        }
      }
      ...
      if (preloadedFilters.categories) {
        ...
        if (Array.isArray(categoriesValue)) {
          ...
          setCategoryFilters(categoriesValue);
        } else {
          setSelectedCategories(new Set([categoriesValue]));
          setCategoryFilters([categoriesValue]);
        }
        ...
        setCategoryFilters([preloadedFilters.category]);
      } else {
        setSelectedCategories(new Set(['ALL']));
      }
  ...
  // Auto-generate title and description when filters are loaded
  useEffect(() => {
    if (!isEditMode &&
  ...
  }, [availableFilters, selectedCategories, countryFilters, filterGroups, statusFilters, isEditMode, preloadedFilters]);
  ...
  const generateReportTitleAndDescription = () => {
    ...
    // 1. Add Status Info
    if (statusFilters.length > 0 && statusFilters.length < 3) {
      const statusNames = statusFilters.map(s => {
        const opt = STATUS_OPTIONS.find(o => o.value === s);
        return opt?.label || s;
      });
      if (statusNames.length === 1) {
        titleParts.push(statusNames[0]);
        descriptionParts.push(`This report includes ${statusNames[0].toLowerCase()} companies.`);
      } else {
        descriptionParts.push(`This report includes ${statusNames.join(' and ').toLowerCase()} companies.`);
      }
    }
    ...
    description += ' Data sourced from Company Database.';
  ...
  const fetchLiveStats = async () => {
    ...
    try {
      ...
      if (countryFilters.length > 0) {
        params.country = countryFilters.join(',');
      }
      ...
      setLiveStats({
        total_records: data.total_companies || 0,
        field_breakdown: {
          categories: data.by_category || [],
          countries: data.by_country?.slice(0, 5) || [],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('💾 Saving company report:', reportData);

      if (isEditMode) {
        // Use existing custom-reports API
  ...
  const handleStatusFilterChange = (newStatusFilters) => {
  ...
  const activeFiltersCount = categoryFilterCount + groupFilterCount + countryFilters.length + (isStatusNonDefault ? 1 : 0);
  ...
  return (
    <DashboardLayout
      pageTitle={isEditMode ? 'Edit Company Report' : 'Create Company Report'}
      pageSubtitleBottom={
        <p className="text-sm text-white-700">
          {isEditMode ? 'Update your company report settings' : 'Define filters to create a custom report from Company Database'}
        </p>
      }
    ...
    >
      ...
      {/* Filter Sidebar - Using CompanyFilterSidebar */}
      {showFilterSidebar && (
                ...
                {currentStep === 1 && (
                  <div className="bg-white rounded-xl shadow-lg border border-gray-200">
                    <div className="border-b border-gray-200 bg-gradient-to-r from-emerald-50 to-teal-50 px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-lg flex items-center justify-center text-white">
                          <Building2 className="w-6 h-6" />
                        </div>
                        <div>
                          <h2 className="text-lg font-semibold text-gray-900">Report Information</h2>
                          <p className="text-xs text-gray-600">Basic details about your company report</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {!isEditMode && Object.keys(preloadedFilters).length > 0 && (
                      <div className="mx-6 mt-4 bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h4 className="text-sm font-semibold text-emerald-900 mb-1">Report Created from Company Database Filters</h4>
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end">
                      <button
                        ...
                        className="bg-emerald-600 text-white px-6 py-2.5 rounded-lg hover:bg-emerald-700 transition-all font-medium flex items-center gap-2"
                      >
                        Next: Filters
                ...
                {currentStep === 2 && (
                  <div className="bg-white rounded-xl shadow-lg border border-gray-200">
                    <div className="border-b border-gray-200 bg-gradient-to-r from-teal-50 to-cyan-50 px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-r from-teal-600 to-cyan-600 rounded-lg flex items-center justify-center text-white">
                          <Filter className="w-6 h-6" />
                        </div>
                        <div>
                          <h2 className="text-lg font-semibold text-gray-900">Filter Criteria</h2>
                          <p className="text-xs text-gray-600">Define which companies to include</p>
                        </div>
                      </div>
                    </div>
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) {
                                      ...
                                      const newFilters = statusFilters.filter(s => s !== option.value);
                            ...
                            {CATEGORIES.filter(cat => cat.value !== 'ALL').map(category => (
                              ...
                              >
                                <input
                                  ...
                                  className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 disabled:opacity-50"
                                />
                          ...
                          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200">
                            <div className="flex items-start justify-between mb-4">
                              ...
                              <button
                                ...
                                className="px-5 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg hover:from-emerald-700 hover:to-teal-700 flex items-center gap-2 shadow-lg transition-all font-medium"
                              >
                            ...
                            {(filterGroups.length > 0 || countryFilters.length > 0 || categoryFilterCount > 0 || isStatusNonDefault) && (
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                  ...
                                  {statusFilters.map(statusValue => {
                                    ...
                                    return (
                                      <span
                                        key={statusValue}
                                        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium border ${status?.color || 'bg-gray-100 text-gray-800'}`}
                                      >
                                        Status: {status?.label || statusValue}
                                        <button
                                          type="button"
                                          onClick={() => {
                                            const newFilters = statusFilters.filter(s => s !== statusValue);
                                            if (newFilters.length > 0) {
                                              setStatusFilters(newFilters);
                                            }
                                          }}
                                          ...
                                          >
                                            ...
                                            <button
                                              ...
                                              onClick={() => {
                                                ...
                                                setCategoryFilters(Array.from(newSelected).filter(c => c !== 'ALL' && c !== 'SELECT'));
                                              }}
                                              className="hover:opacity-70"
                                            >
                                              <X className="w-4 h-4" />
                                  ...
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                      <h4 className="text-sm font-bold text-emerald-900 mb-2 flex items-center gap-2">
                                        ...
                                        <span className="text-xs bg-emerald-200 text-emerald-800 px-2 py-0.5 rounded-full font-normal">
                                          Live
                                        </span>
                                      </h4>
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-between">
                      ...
                      <button
                        ...
                        className="bg-teal-600 text-white px-6 py-2.5 rounded-lg hover:bg-teal-700 transition-all font-medium flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next: Review
                        <ChevronRight className="w-4 h-4" />
                      </button>
                    ...
                    <div className="p-6 space-y-6">
                      ...
                      <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg p-4 border border-emerald-200">
                        ...
                        <div className="space-y-3 text-sm">
                          ...
                          {statusFilters.length > 0 && (
                            <div>
                              <span className="text-xs text-gray-600">Company Status ({statusFilters.length}):</span>
                              <div className="flex flex-wrap gap-1 mt-1">
                                {statusFilters.map(statusValue => {
                                  const status = STATUS_OPTIONS.find(s => s.value === statusValue);
                                  return (
                                    <span key={statusValue} className={`text-xs px-2 py-1 border rounded-full ${status?.color || 'bg-gray-100 text-gray-700'}`}>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1 mt-1">
                                {Array.from(selectedCategories).map(categoryValue => {
                                  ...
                                  return (
                                    <span key={categoryValue} className="text-xs px-2 py-1 bg-white border border-purple-300 text-purple-800 rounded-full">
                            ...
                            <div>
                              ...
                              <div className="mt-2 space-y-2">
                                {filterGroups.map((group) => {
                                  ...
                                  return (
                                    <div key={group.id} className="bg-white rounded-lg p-2 border border-emerald-200">
                                      <div className="text-xs font-semibold text-emerald-700 mb-1">{group.name}</div>
                          ...
                          {activeFiltersCount === 0 && (
                            <p className="text-sm text-gray-500 italic">No filters applied - all companies will be included</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg p-4 border border-emerald-200">
                        ...
                        <div className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-600">
                          {liveStats.total_records.toLocaleString()} companies
                        </div>
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden sticky top-4">
                  <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-5 py-4 text-white">
                    ...
                    <p className="text-xs text-emerald-100 mt-1">Company Database</p>
                  </div>
                  <div className="p-5">
                    {statsLoading ? (
                      ...
                      <div className="space-y-4">
                        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-5 rounded-xl border border-emerald-200">
                          ...
                          <div className="text-sm font-medium text-gray-600 mt-1">Total Companies</div>
                        </div>

                        {liveStats.field_breakdown?.status && (
                          <div className="bg-gray-50 rounded-lg p-4">
                            <h4 className="text-xs font-semibold text-gray-700 mb-3 uppercase tracking-wide">By Status</h4>
                            <div className="space-y-2">
                              {liveStats.field_breakdown.status.complete > 0 && (
                                <div className="flex justify-between items-center text-sm">
                                  <span className="text-gray-600">Complete</span>
                                  <span className="font-semibold text-green-700">{liveStats.field_breakdown.status.complete}</span>
                                </div>
                              )}
                              {liveStats.field_breakdown.status.incomplete > 0 && (
                                <div className="flex justify-between items-center text-sm">
                                  <span className="text-gray-600">Incomplete</span>
                                  <span className="font-semibold text-yellow-700">{liveStats.field_breakdown.status.incomplete}</span>
                                </div>
                              )}
                              {liveStats.field_breakdown.status.deleted > 0 && (
                                <div className="flex justify-between items-center text-sm">
                                  <span className="text-gray-600">Deleted</span>
                                  <span className="font-semibold text-red-700">{liveStats.field_breakdown.status.deleted}</span>
                                </div>
                              )}
                            </div>
                          </div>
                  ...
                  <div className="p-5 border-t border-gray-200 bg-gray-50 space-y-3">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-5 py-3 rounded-lg hover:from-emerald-700 hover:to-teal-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-medium shadow-lg flex items-center justify-center gap-2"
                    >
                      {loading ? (
                        <>
                          <LoadingSpinner />
...
export default CreateCompanyReportPage;
