...
/**
 * Filter Sidebar for Company Database
 * 
 * This is based on FilterSidebarWithGroups.jsx but adds:
...
import { X, Search, Plus, Trash2, Copy, ChevronDown, ChevronUp, Check, AlertCircle } from 'lucide-react';
import { CATEGORIES } from '../../constants/categories';

// Status configuration
const STATUS_OPTIONS = [
  { value: 'COMPLETE', label: 'Complete', icon: Check, color: 'bg-green-100 text-green-800 border-green-300' },
  { value: 'INCOMPLETE', label: 'Incomplete', icon: AlertCircle, color: 'bg-yellow-100 text-yellow-800 border-yellow-300' },
  { value: 'DELETED', label: 'Deleted', icon: Trash2, color: 'bg-red-100 text-red-800 border-red-300' }
];
...
const CompanyFilterSidebar = ({
  ...
  const toggleStatus = (status) => {
    ...
    if (statusFilters.includes(status)) {
      newStatusFilters = statusFilters.filter(s => s !== status);
    } else {
      newStatusFilters = [...statusFilters, status];
    }
  ...
  // Toggle category filter with "All Categories" logic
  ...
  }, 0) + countryFilters.length + categoryFilters.length + statusFilters.length;
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full md:w-[700px] bg-white shadow-2xl z-50 flex flex-col animate-slide-in">
        ...
        <div className="border-b p-4 bg-gray-50 flex-shrink-0">
          ...
          {(statusFilters.length > 0 || categoryFilters.length > 0 || countryFilters.length > 0 ||
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => {
                    ...
                    onCategoryFilterChange([]);
                    onCountryFilterChange([]);
                    onStatusFilterChange(['COMPLETE', 'INCOMPLETE']);
                  }}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {statusFilters.map((status) => {
                  ...
                  return (
                    <span
                      key={status}
                      className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium border ${statusOption?.color || 'bg-gray-100 text-gray-800'}`}
                    >
          ...
          <div className="mb-4 border border-gray-200 rounded-lg overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                <h4 className="font-semibold text-gray-900 text-sm">Company Status</h4>
                  ...
                  {STATUS_OPTIONS.map(option => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <Icon className={`w-4 h-4 ${isChecked ? '' : 'text-gray-500'}`} />
